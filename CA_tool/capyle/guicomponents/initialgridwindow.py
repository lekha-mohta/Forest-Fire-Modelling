...
class _ConfigFrame(tk.Frame):
    ...
    def add_paint_states(self, parent):
        ...
        container = tk.Canvas(parent, width = 75, height = 700)
        scrollbar = Scrollbar(parent, orient=tk.VERTICAL, command=container.yview)
        container.configure(yscrollcommand=scrollbar.set)

        innerFrame = tk.Frame(container)
        # Add the label, radio button and color for each state
        for i, state in enumerate(self.ca_config.states):
            frame = tk.Frame(innerFrame)
            label = tk.Label(frame, text=state)
            label.pack(side=tk.LEFT)
            color = rgb_to_hex(*self.ca_config.state_colors[i])
        ...
        innerFrame.pack(fill=tk.BOTH, expand=tk.YES)
        container.create_window((0,0), window=innerFrame, anchor="n")
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)
        container.pack()
        innerFrame.bind("<Configure>", lambda e: container.configure(scrollregion=container.bbox("all")))
