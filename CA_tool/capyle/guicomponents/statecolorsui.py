...
class _StateColorsUI(tk.Frame, _ConfigUIComponent):
    ...
    def __init__(self, parent, ca_config, ca_graph):
        ...
        outerframe = tk.Canvas(self, width = 150)
        scroll = Scrollbar(self, orient=tk.VERTICAL, command=outerframe.yview)
        outerframe.configure(yscrollcommand=scroll.set)

        #secondFrame = tk.Frame(outerframe, width = 75)        
        for i, state in enumerate(self.states):
            ...
            if i % 3 == 2 and not (i == len(self.states) - 1):
                outerframe.pack()
                outerframe = tk.Canvas(self)
        
        #secondFrame.pack(fill=tk.BOTH, expand=tk.YES)
        # outerframe.create_window((0,0), window=secondFrame, anchor="nw")
        scroll.pack(side=tk.RIGHT, fill=tk.Y)
        outerframe.pack()
        #secondFrame.bind("<Configure>", lambda e: outerframe.configure(scrollregion=outerframe.bbox("all")))
